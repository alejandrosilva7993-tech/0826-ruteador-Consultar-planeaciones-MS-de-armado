let plansPageSize = 25;
let routesPageSize = 25;
let discardedPageSize = 25;
const ROUTES_COLSPAN = 12;
...
function countSplitOrders(route) {
  return route.stops.reduce(
    (sum, stop) => sum + stop.orders.filter((order) => order.isChild).length,
    0
  );
}

function armadoChip(value) {
  const isDone = value >= 100;
  const modifier = isDone ? "chip-avance-done" : "chip-avance-pending";
  const label = isDone ? "Completado" : "Pendiente";
  return `<span class="chip-avance ${modifier}">${label}</span>`;
}
...
function renderRow(plan) {
  return `
    <tr>
      ...
      <td>${armadoChip(plan.progress)}</td>
      <td>
...
function totalPages() {
  return Math.max(1, Math.ceil(PLANS.length / plansPageSize));
}

function occupancyProgress(occupancy) {
  const value = Math.max(0, Math.min(100, parseInt(String(occupancy), 10) || 0));
  return { value, fillClass: progressClass(value), widthAttr: progressWidth(value) };
}
...
function renderOrderRow(order) {
  ...
  const parentWeight = isChild ? order.parentWeight : "No aplica";
  const parentVolume = isChild ? order.parentVolume : "No aplica";
  return `
...
function renderRouteRow(route, index) {
  ...
  const occ = occupancyProgress(route.occupancy);
  const main = `
    <tr class="route-row ${zebra} ${isExpanded ? "is-expanded" : ""} ${route.id === activeRouteId ? "is-active" : ""}" data-route-id="${route.id}">
      <td class="td-expand">
        <button
          type="button"
      ...
      <td class="td-occupancy">
        <div class="progress progress-compact">
          <span class="progress-label">${occ.value}%</span>
          <div class="progress-track">
            <div class="progress-fill ${occ.fillClass}" ${occ.widthAttr}></div>
          </div>
        </div>
      </td>
...
function updateRoutesSelectionHint() {
  ...
  const bulkAccept = document.getElementById("btn-bulk-accept");
  ...
  if (bulkAccept) bulkAccept.disabled = !hasSelection;
...
function renderActiveRouteId() {
  const target = document.getElementById("detail-route-id");
  if (target) target.textContent = activeRouteId || "";
}
...
function renderRoutesTable() {
  ...
  if (!slice.some((route) => route.id === activeRouteId)) {
    activeRouteId = slice.length ? slice[0].id : null;
  }
  renderActiveRouteId();
...
function renderDetail(planName) {
  ...
  activeRouteId = currentRoutes.length ? currentRoutes[0].id : null;

  const totalOrders = currentRoutes.reduce((sum, route) => sum + countOrders(route), 0);
  const splitOrders = currentRoutes.reduce((sum, route) => sum + countSplitOrders(route), 0);
  const discardedTotal = DISCARDED_ORDERS.length;
  const selectedTotal = totalOrders + discardedTotal;
  ...
  document.querySelector('[data-key="split"]').textContent = String(splitOrders);
  document.querySelector('[data-key="discarded"]').textContent = String(discardedTotal);
...
function clearFilters() {
  document.getElementById("fecha-inicio").value = FILTER_DEFAULTS.fechaInicio;
  document.getElementById("fecha-fin").value = FILTER_DEFAULTS.fechaFin;
  document.getElementById("nombre-plan").value = "";
  currentPage = 1;
  renderTable();
}

function scrollToDiscarded(event) {
  event.preventDefault();
  const target = document.getElementById("card-discarded");
  if (!target) return;
...
function showList() {
  ...
  pageSubtitle.textContent = "Consultar planeaciones generadas";
...
document.getElementById("plans-page-size").addEventListener("change", (event) => {
  plansPageSize = Number(event.target.value) || 25;
  currentPage = 1;
  renderTable();
});
...
document.getElementById("routes-page-next").addEventListener("click", () => {
  const pages = Math.max(1, Math.ceil(currentRoutes.length / routesPageSize));
  if (routesPage < pages) {
    routesPage += 1;
    renderRoutesTable();
  }
...
document.getElementById("routes-page-size").addEventListener("change", (event) => {
  routesPageSize = Number(event.target.value) || 25;
  routesPage = 1;
  renderRoutesTable();
});
...
document.getElementById("discarded-page-next").addEventListener("click", () => {
  const pages = Math.max(1, Math.ceil(DISCARDED_ORDERS.length / discardedPageSize));
...
document.getElementById("discarded-page-size").addEventListener("change", (event) => {
  discardedPageSize = Number(event.target.value) || 25;
  discardedPage = 1;
  renderDiscardedTable();
});
...
document.getElementById("indicator-discarded-link").addEventListener("click", scrollToDiscarded);
...
document.getElementById("routes-tbody").addEventListener("click", (event) => {
  const row = event.target.closest("tr.route-row[data-route-id]");
  const isCheckbox = event.target.matches("input[data-route-check]");
  const activeChanged = Boolean(row) && !isCheckbox && row.dataset.routeId !== activeRouteId;
  if (activeChanged) activeRouteId = row.dataset.routeId;
  ...
  if (activeChanged) renderRoutesTable();

  const acceptBtn = event.target.closest("[data-accept]");
  if (acceptBtn) {
    openConfirmModal({
      action: "accept",
      routeIds: [acceptBtn.dataset.accept],
    });
    return;
  }
  ...
  if (rejectBtn) {
    openConfirmModal({
      action: "reject",
      routeIds: [rejectBtn.dataset.reject],
    });
  }
});
...
document.getElementById("btn-bulk-accept").addEventListener("click", () => {
  ...
  openConfirmModal({ action: "accept", routeIds: ids });
});

document.getElementById("btn-bulk-reject").addEventListener("click", () => {
  const ids = getSelectedRouteIds();
  if (!ids.length) return;
  openConfirmModal({ action: "reject", routeIds: ids });
});

let pendingConfirm = null;

function openConfirmModal({ action, routeIds }) {
  const modal = document.getElementById("confirm-modal");
  const titleEl = document.getElementById("confirm-modal-title");
  const leadEl = document.getElementById("confirm-modal-lead");
  const bodyEl = document.getElementById("confirm-modal-body");
  const plural = routeIds.length > 1;

  pendingConfirm = { action, routeIds };

  if (action === "accept") {
    titleEl.textContent = plural ? "Aceptar rutas" : "Aceptar ruta";
    leadEl.textContent = "";
    bodyEl.innerHTML = plural
      ? "<p>Las órdenes quedarán asignadas y las rutas no podrán ser modificadas. Al aceptar las rutas, si estas incluyen órdenes hijas, se ejecutará la división de la orden original y se crearán dichas órdenes.</p><p><strong>¿Deseas continuar?</strong></p>"
      : "<p>Las órdenes quedarán asignadas y la ruta no podrá ser modificada. Al aceptar la ruta, si esta incluye órdenes hijas, se ejecutará la división de la orden original y se crearán dichas órdenes.</p><p><strong>¿Deseas continuar?</strong></p>";
  } else {
    titleEl.textContent = plural ? "Rechazar rutas" : "Rechazar ruta";
    leadEl.textContent = "";
    bodyEl.innerHTML = plural
      ? "<p>Las órdenes quedarán disponibles para nuevas planeaciones.</p><p><strong>¿Deseas continuar?</strong></p>"
      : "<p>Las órdenes quedarán disponibles para nuevas planeaciones.</p><p><strong>¿Deseas continuar?</strong></p>";
  }
...
function confirmPendingAction() {
  ...
  const { action, routeIds } = pendingConfirm;
  closeConfirmModal();
  const plural = routeIds.length > 1;
  if (action === "accept") {
    openSuccessModal(plural ? "Las rutas han sido aceptadas." : "La ruta ha sido aceptada.");
  } else {
    openSuccessModal(plural ? "Las rutas han sido rechazadas." : "La ruta ha sido rechazada.");
  }
}

function openSuccessModal(message) {
  const modal = document.getElementById("success-modal");
  document.getElementById("success-modal-title").textContent = "¡Listo!";
...
document.getElementById("confirm-modal-continue").addEventListener("click", confirmPendingAction);

document.getElementById("confirm-modal").addEventListener("click", (event) => {
  if (event.target.id === "confirm-modal") closeConfirmModal();
});

document.getElementById("success-modal-ok").addEventListener("click", closeSuccessModal);
